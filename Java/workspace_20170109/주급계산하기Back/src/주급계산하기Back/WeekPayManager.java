...
public class WeekPayManager {
    ...
    public void input()
    {
    	...
    	do
    	{
    		...
    		sc.nextLine();//int값이나 float값 읽고나서 쓰자!
    		again = sc.nextLine();		
    	}while(again.equals("1"));
    	
    	count = i;//현재 데이터 개수 저장하기
    }
    ...
    void process()
    {
    	...
    	{
    		//기본급
    		data[i].setBase_pay(data[i].getHour_pay()*data[i].getWork_time());
    		...
    		data[i].setReal_pay(data[i].getTotal_pay()-data[i].getTax());
    	}
    }
    void output()
    {
